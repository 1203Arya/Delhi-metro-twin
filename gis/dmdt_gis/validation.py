...
def _in_delhi(lat: float, lon: float) -> bool:
    return DELHI_LAT_MIN <= lat <= DELHI_LAT_MAX and DELHI_LON_MIN <= lon <= DELHI_LON_MAX
...
def validate_station(station: StationSpec) -> ValidationResult:
    errors: list[str] = []
    warnings: list[str] = []
    cv = validate_coordinate(station.latitude, station.longitude, f"station {station.code}")
    errors.extend(cv.errors)
    warnings.extend(cv.warnings)
    if station.structure not in ("elevated", "underground", "at-grade"):
        errors.append(f"station {station.code}: invalid structure {station.structure!r}")
    if station.platforms < 1:
        errors.append(f"station {station.code}: platforms must be >= 1")
    if station.platforms > 8:
        warnings.append(f"station {station.code}: unusually high platform count ({station.platforms})")
    if station.opened_year < 2000:
        warnings.append(f"station {station.code}: opened_year {station.opened_year} seems early")
    if station.opened_year > 2030:
        warnings.append(f"station {station.code}: opened_year {station.opened_year} in the future")
    return ValidationResult(valid=not errors, errors=tuple(errors), warnings=tuple(warnings))
...
def validate_line(line: LineSpec) -> ValidationResult:
    ...
    for idx, station in enumerate(line.stations):
        ...
        if idx > 0:
            ...
            dist_m = haversine_m((prev.latitude, prev.longitude), (station.latitude, station.longitude))
    ...
    valid_line_codes = ...  # populated later
    return ValidationResult(valid=not errors, errors=tuple(errors), warnings=tuple(warnings))
...
def validate_network(network: Network) -> ValidationResult:
    ...
    for line in network.lines:
        ...
        for ic_station, ic_line, _ic_with in line.interchanges:
            if ic_line not in line_codes:
                errors.append(f"line {line.code}: interchange references unknown line {ic_line!r}")
            if ic_line == line.code:
                warnings.append(f"line {line.code}: self-referencing interchange at {ic_station}")
    station_coords: dict[str, list[tuple[str, float, float]]] = {}
    for ln in network.lines:
        for s in ln.stations:
            station_coords.setdefault(s.code, []).append((ln.code, s.latitude, s.longitude))
    for code, entries in station_coords.items():
        ...
        if len(lat_set) > 1 or len(lon_set) > 1:
            ...
            warnings.append(f"station {code} has inconsistent coordinates across lines: {reprs}")
    return ValidationResult(valid=not errors, errors=tuple(errors), warnings=tuple(warnings))
...
def validate_station_polygon(
    ...
    if not poly.is_valid:
        errors.append(f"station {station.code}: polygon invalid ({is_valid_reason(poly)})")
    if poly.area <= 0:
...
def validate_platform_polygon(
    ...
    if length_m <= 0 or length_m > MAX_PLATFORM_LENGTH_M:
        errors.append(f"platform {station.code}-{platform_no}: length {length_m} out of range")
    if width_m <= 0 or width_m > MAX_PLATFORM_WIDTH_M:
        errors.append(f"platform {station.code}-{platform_no}: width {width_m} out of range")
    ...
    if not poly.is_valid:
        errors.append(f"platform {station.code}-{platform_no}: polygon invalid ({is_valid_reason(poly)})")
    if poly.area <= 0:
...
def validate_track_segment(seg: TrackSegmentGeometry) -> ValidationResult:
    ...
    warnings: list[str] = []
    if seg.length_m <= 0:
        errors.append(f"segment {seg.from_station}->{seg.to_station}: non-positive length")
    if not (0 <= seg.heading_in_deg < 360):
        warnings.append(f"segment {seg.from_station}->{seg.to_station}: heading_in {seg.heading_in_deg} out of range")
    if not (0 <= seg.heading_out_deg < 360):
        warnings.append(f"segment {seg.from_station}->{seg.to_station}: heading_out {seg.heading_out_deg} out of range")
    if seg.speed_limit_kmh < 5:
        errors.append(f"segment {seg.from_station}->{seg.to_station}: speed limit {seg.speed_limit_kmh} too low")
    if seg.speed_limit_kmh > 200:
        errors.append(f"segment {seg.from_station}->{seg.to_station}: speed limit {seg.speed_limit_kmh} implausible")
    ...
    if seg.is_curve and (seg.max_curve_radius_m is None or seg.max_curve_radius_m < 50):
        errors.append(f"segment {seg.from_station}->{seg.to_station}: curve radius {seg.max_curve_radius_m}m too tight")
    coords = list(seg.coords)
    if len(coords) < 2:
        errors.append(f"segment {seg.from_station}->{seg.to_station}: insufficient coordinates")
    if len(coords) > 2:
    ...
    if seg.direction not in ("up", "down"):
        errors.append(f"segment {seg.from_station}->{seg.to_station}: invalid direction {seg.direction!r}")
    return ValidationResult(valid=not errors, errors=tuple(errors), warnings=tuple(warnings))
