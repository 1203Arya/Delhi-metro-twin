...
class PassengerFlowModel:
    ...
    def calc_dwell_time(
        self, boarding: int, alighting: int, base_dwell: float | None = None
    ) -> float:
    ...
    def calc_walk_time(
        self, from_platform: str, to_platform: str, station_data: dict[str, Any]
    ) -> float:
        return 60.0

    def calc_transfer_time(
        self, from_line: str, to_line: str, station_code: str
    ) -> float:
        return 120.0
    ...
    def should_reroute_due_to_crowding(
        self, occupancy: int, capacity: int, threshold: float = 0.9
    ) -> bool:
        if capacity <= 0:
            return False
