...
class CBTCController:
    def __init__(self, block_manager: BlockManager, min_headway_m: float = 50.0) -> None:
    ...
    def compute_movement_authority(
        ...
        brake_dist = (current_speed_mps * current_speed_mps) / (2 * train_spec.deceleration_ms2)
        safe_stop_dist = brake_dist * self.safe_braking_distance_factor
        lead_id, gap = self.block_manager.get_leading_train(line_code, direction, current_position_m, train_spec.length_m)
    ...
    def compute_target_speed(
        ...
        if authority == MovementAuthority.RESTRICTED:
            safe_speed = (2 * train_spec.deceleration_ms2 * distance_to_obstruction_m) ** 0.5
    ...
    def check_conflicts(self, line_code: str, direction: Direction) -> list[dict[str, Any]]:
        conflicts: list[dict[str, Any]] = []
        blocks = self.block_manager._line_blocks.get(line_code, {}).get(direction.value, [])
        prev_occupied = False
        for block in blocks:
